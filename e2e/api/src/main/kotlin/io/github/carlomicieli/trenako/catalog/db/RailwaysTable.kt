...
@Component
class RailwaysTable(private val r2dbcEntityTemplate: R2dbcEntityTemplate) {
    ...
    suspend fun selectByName(name: String): RailwayRow? {
        return r2dbcEntityTemplate
            .selectOne(query(where("name").`is`(name)), ENTITY)
            .awaitSingleOrNull()
    }

    suspend fun insert(row: RailwayRow) {
        r2dbcEntityTemplate.insert(row).awaitSingle()
    }

    companion object {
        private val ENTITY: Class<RailwayRow> = RailwayRow::class.java
    }
}
